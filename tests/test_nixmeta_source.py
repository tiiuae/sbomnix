...
def test_missing_packaged_meta_nix_in_scanner_fails_cleanly(monkeypatch):
    """A missing packaged meta.nix must not crash scanner-backed metadata enrichment."""

    def missing_meta_nix():
        raise FileNotFoundError("missing packaged meta.nix")

    monkeypatch.setattr(nixmeta_scanner, "meta_nix_path", missing_meta_nix)
    ...
    df_meta, source = sbomnix_meta.Meta()._scan_source_with_source(
        source,
        store_names=_NAMES,
    )
    ...
    assert source.method == "flake-meta"
    assert source.message == "meta.nix scan failed. Skipping nixpkgs metadata."


def test_missing_packaged_meta_cache_fingerprint_falls_back_to_missing(monkeypatch):
    """A missing packaged meta.nix must not crash cache-key invalidation."""

    def missing_meta_nix():
        raise FileNotFoundError("missing packaged meta.nix")

    monkeypatch.setattr(sbomnix_meta, "meta_nix_path", missing_meta_nix)
    sbomnix_meta._meta_cache_fingerprint.cache_clear()
    try:
        assert sbomnix_meta._meta_cache_fingerprint() == "missing"
    finally:
        sbomnix_meta._meta_cache_fingerprint.cache_clear()
...
def test_scan_source_filters_non_package_artifact_names(monkeypatch):
    """Patch/source artifact names must not be forwarded to exact metadata lookup."""
    scanned = []
    fake_df = SimpleNamespace(empty=False)
    source = sbomnix_meta.NixpkgsMetaSource(
        method="flake-meta",
        pkgs_expression="pkgs",
        expression_cache_key="flake-meta:path:/nix/store/abc#target:bt",
    )
    ...
    assert df_meta is fake_df
    assert out_source.message is None
    assert scanned == [["hello-2.12.3"]]
...
def test_scan_source_filters_patch_and_diff_names_before_scanning(monkeypatch):
    scanned = []
    fake_df = SimpleNamespace(empty=False)
    ...
    monkeypatch.setattr(
        ...
        "_scan_store_names",
        lambda self, names, *, cache_key=None, impure=False, pkgs_expr=None: (
            scanned.append(list(names)) or fake_df
        ),
    )
    ...
    assert scanned == [["hello-2.12.3", "glibc-2.42"]]


def test_scan_source_cache_key_ignores_patch_and_diff_names(monkeypatch):
    scan_calls = []
    ...
    names_with_artifacts = names + ["fix-build.patch", "series.diff.gz"]

    meta._scan_source_with_source(source, store_names=names_with_artifacts)
    meta._scan_source_with_source(source, store_names=names)

    assert scan_calls == [names]
...
def test_scan_store_names_cache_key_includes_meta_cache_fingerprint(monkeypatch):
    """Changing metadata lookup code must invalidate old cache entries."""
    scan_calls = []
    ...
    monkeypatch.setattr(meta.cache, "set", fake_set)
    monkeypatch.setattr(sbomnix_meta, "NixMetaScanner", FakeScanner)
    monkeypatch.setattr(sbomnix_meta, "_meta_cache_fingerprint", lambda: "hash-a")
    ...
    monkeypatch.setattr(sbomnix_meta, "_meta_cache_fingerprint", lambda: "hash-b")
    meta._scan_store_names(names, cache_key=cache_key, pkgs_expr="pkgs")
    assert len(scan_calls) == 2, (
        "A changed metadata lookup fingerprint must invalidate the previous cache entry"
    )
...
def test_third_party_flake_relative_path_locked_nixpkgs_uses_parent_source(
    monkeypatch,
):
    """A relative path-locked nixpkgs input must resolve against the parent source."""
    source_path = "/nix/store/abc-src"
    scanned = []
    version_reads = []
    fake_df = SimpleNamespace(empty=False)
    ...
    expected_path = f"{source_path}/nixpkgs"
    assert df_meta is fake_df
    assert source.method == "flake-meta"
    assert source.flakeref == f"path:{expected_path}"
    assert source.path == expected_path
    assert source.version == "24.11pre"
    assert version_reads == [expected_path]
    assert len(scanned) == 1
...
def test_third_party_flake_relative_path_locked_nixpkgs_uses_parent_chain(
    monkeypatch,
):
    """Nested relative path inputs must resolve against the parent lock chain."""
    source_path = "/nix/store/abc-src"
    scanned = []
    version_reads = []
    fake_df = SimpleNamespace(empty=False)
    ...
    lock_graph = {
        ...
        "version": 7,
        "nodes": {
            "root": {"inputs": {"foo": "foo", "nixpkgs": ["foo", "nixpkgs"]}},
            "foo": {
    ...
    expected_path = f"{source_path}/vendor/foo-src/nixpkgs"
    assert df_meta is fake_df
    assert source.method == "flake-meta"
    assert source.flakeref == f"path:{expected_path}"
    assert source.path == expected_path
...
def test_third_party_flake_relative_path_locked_nixpkgs_parent_chain_honors_dir(
    monkeypatch,
):
    """Nested relative path inputs must resolve from the parent flake subdirectory."""
    source_path = "/nix/store/abc-src"
    scanned = []
    version_reads = []
    fake_df = SimpleNamespace(empty=False)
    ...
    expected_path = f"{source_path}/src/subflake/nixpkgs"
    assert df_meta is fake_df
    assert source.method == "flake-meta"
    assert source.flakeref == f"path:{expected_path}"
    assert source.path == expected_path
...
def test_third_party_flake_relative_path_locked_nixpkgs_under_git_parent(
    monkeypatch,
):
    """A relative path child under a git parent must resolve via parent flakeref."""
    source_path = "/nix/store/abc-src"
    git_url = "https://git.example.com/foo"
    git_ref = "refs/heads/main"
    git_rev = "deadbeef"
    git_nar = "sha256-foohash"
    scanned = []
    fake_df = SimpleNamespace(empty=False)
...
def test_third_party_flake_relative_path_locked_nixpkgs_under_git_parent_dir(
    monkeypatch,
):
    """A relative path child under a git parent must extend the parent's dir=."""
    source_path = "/nix/store/abc-src"
    git_url = "https://git.example.com/foo"
    git_rev = "deadbeef"
    git_dir = "subflake"
    ...
    expected_ref = f"git+{git_url}?rev={git_rev}&dir=subflake/nixpkgs"
    expected_pkgs = f"import (builtins.getFlake {json.dumps(expected_ref)}) {{}}"
    assert df_meta is fake_df
    assert source.method == "flake-meta"
    assert source.flakeref == expected_ref
