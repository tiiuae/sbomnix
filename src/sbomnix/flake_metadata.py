...
def normalize_current_flake_shorthand(flakeref):
    """Return ``.#attr`` for valid current-flake shorthand ``#attr``."""
    flakeref = str(flakeref or "")
    flake, separator, attr = flakeref.partition("#")
    if separator and not flake:
        return f".{separator}{attr}"
    return flakeref


def normalize_local_flake_ref(flake):
    """Return an absolute local flake ref, leaving registry and URL refs intact."""
...
def normalize_flakeref_for_nix_eval(flakeref):
    """Normalize current-flake shorthand and local paths before Nix evaluation."""
    flakeref = normalize_current_flake_shorthand(flakeref)
    flake, separator, attr = flakeref.partition("#")
    if not flake:
        return flakeref
    return f"{normalize_local_flake_ref(flake)}{separator}{attr}"
