...
def _eval_flakeref_path(flakeref, *, impure, exec_cmd_fn, log):
    """Return the output path string for a flakeref target."""
    log.info("Evaluating flakeref '%s'", flakeref)
    cmd = nix_cmd("eval", "--raw", flakeref, impure=impure)
    ret = exec_cmd_fn(cmd, raise_on_error=False, return_error=True, log_error=False)
    if ret is None or ret.returncode != 0:
...
def _store_path_exists(path: str) -> bool:
    """Return True when ``path`` is an existing local Nix store path."""
    return (
        isinstance(path, str)
