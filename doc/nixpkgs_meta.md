...
name is a useful hint toward the correct derivation, but it is not a
proof of package identity.  The lookup is heuristic: it strips version
suffixes with regex patterns, reorders search sets to prefer
language-specific package sets when a language prefix is detected, and
applies naming-convention fallbacks for packages whose nixpkgs
attribute name diverges from the pname.  Pname collisions across
package sets are reduced rather than ruled out by construction.

Important limitation: metadata lookup can assign incorrect metadata
when multiple derivations share similar names, when a package uses
unusual naming conventions, or when a fallback finds a plausible but
wrong attribute. Ambiguity detection avoids some unsafe joins, but it
...
name suffix. Here `name` means the full store-path suffix such as
`hello-2.12.3`, not just the bare `pname` `hello`.
...
list and passes it in batches to `meta.nix`. The same full `name`
string, including the version suffix, is also the join key when
metadata is merged back into the SBOM. The lookup is not merged only on
bare package names like `hello` or `glibc`.
...
Some store-path names resolve to more than one derivation in the first
matching heuristic tier, for example when the same pname exists both at
the top level and inside a sub-package set. `meta.nix` marks two
internal flags for those cases:

- `ambiguous`: more than one candidate matched in the winning tier
- `preciseNeeded`: the exported metadata differs between those candidates

`sbomnix` only merges metadata into the SBOM when the name-only result
is safe to keep:

- if candidates are metadata-equivalent, the first result is reused
- if candidates differ in exported metadata, the package name is left
  blank instead of guessing

These flags are used only to control the merge policy. They are not
exported as SBOM metadata columns.

This ambiguity handling is conservative, not complete. A metadata row
without `preciseNeeded` still means "best heuristic match", not an
authoritative reverse mapping from store path to nixpkgs attribute.
